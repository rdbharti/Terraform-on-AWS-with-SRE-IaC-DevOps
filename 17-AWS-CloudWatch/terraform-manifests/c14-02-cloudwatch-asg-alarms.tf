# Define CloudWatch Alarm for Autoscaling Group
...
# Also send the notification email to users present in SNS Topic Subscription
...
resource "aws_cloudwatch_metric_alarm" "app1_asg_cwa_cpu" {
  ...
  alarm_actions     = [aws_sns_topic.myasg_sns_topic.arn, aws_autoscaling_policy.high_cpu.arn]
}